...
export default class Chat extends Component {
  ...
  componentWillMount() { 
    ...
    if (cookieArray.length > 0) {
      for (var i = 0; i < cookieArray.length; i++) {
        ...
        var KeyValue = currentcookie.split("=");
  ...
  render() {
    ...
    const dropDownMenu = (
      <div className="ddr_width">
        <DropdownButton key={1} id="user-menu" bsSize="large" className="left_btn" title={username}>
          <MenuItem style={{ 'width': '100%' }} eventKey="4" onSelect={::this.handleSignOut}>Sign out</MenuItem>
        </DropdownButton>
      </div >
    );
    ...
    return (
      <div style={{ margin: '0', padding: '0', height: '100%', width: '100%', display: '-webkit-box' }}>
        ...
        <div className="main">
          ...
          {PrivateMessageModal}
          <ul ref="messageList" className="msg_list">
            {filteredMessages.map(message =>
              <MessageListItem currentuser={username} handleClickOnUser={::this.handleClickOnUser} message={message} key={message.id} />
            )}
          </ul>
